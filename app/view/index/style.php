...
<div class="page-banner"><img src="<?= htmlspecialchars($assetUrl($banner ?? ''), ENT_QUOTES, 'UTF-8') ?>" class="w100 block" alt="员工风采"></div>
...
<main class="about-container center mt20 mb60 border-radius-5 fs-14 clearfix">
    <ul class="photo_list clearfix mt30">
        <?php foreach ($staffPhotos as $photo): ?>
            <li><a class="block" href="<?= htmlspecialchars($photo['image'] ?? '#', ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener"><div class="img"><img src="<?= htmlspecialchars($photo['image'] ?? '', ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($photo['title'] ?? '众人搬家员工风采', ENT_QUOTES, 'UTF-8') ?>"></div><p><?= htmlspecialchars($photo['title'] ?? '', ENT_QUOTES, 'UTF-8') ?></p></a></li>
        <?php endforeach; ?>
    </ul>
</main>
