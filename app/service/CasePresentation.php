...
class CasePresentation
{
    public static function prepare(array $case): array
    {
        ...
        $fields = [];
        foreach (['服务地点', '服务类型', '服务时间', '项目难点', '众人搬家解决方案'] as $label) {
            $collect = false;
            $parts = [];
            foreach ($lines as $line) {
                ...
                if ($collect && preg_match('/^(客户痛点|项目难点|众人搬家解决方案|解决方案|交付成果|服务优势|服务地点|服务类型|服务时间)(?:[：:]|$)/u', $line)) break;
        ...
        if ($summary === '' || str_contains($summary, '这里可查询') || str_contains($summary, '哪家靠谱')) {
            ...
            if (!$fields) $summary .= '查看众人搬家展示的项目图片。';
        }
        $case['title'] = $title;
        $case['headline'] = '众人搬家案例：' . $title;
