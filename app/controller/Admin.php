...
class Admin extends BaseController
{
    ...
    public function login()
    {
        ...
        $notice = $this->flash();
        $body = '<div class="login-card"><h1>众人搬家后台</h1><p>内容与线索管理</p>'
    ...
    private function page($title, $body, $withNav = true)
    {
        ...
        $nav = '';
        if ($withNav) {
            $nav = '<aside><a class="brand" href="' . $this->url('/dashboard') . '">众人搬家后台</a><a href="' . $this->url('/dashboard') . '">工作台</a><a href="' . $this->url('/messages') . '">留言线索</a><a href="' . $this->url('/articles') . '">新闻资讯</a><a href="' . $this->url('/casesManage') . '">服务案例</a><a href="' . $this->url('/settings') . '">站点配置</a><form method="post" action="' . $this->url('/logout') . '">' . $this->csrfInput() . '<button class="logout">退出登录</button></form></aside>';
        }
        $notice = $flash ? '<div class="notice success">' . $this->e($flash) . '</div>' : '';
        echo '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . $this->e($title) . ' - 众人搬家后台</title><style>' . $this->styles() . '</style></head><body class="' . ($withNav ? '' : 'login') . '">' . $nav . '<main>' . $notice . $body . '</main></body></html>';
    }
