...
```bash
cd zhongren_guanwang
composer install --no-dev --optimize-autoloader
```

### 3. 配置数据库

默认配置直接使用 `data/demo.sqlite`。正式上线前应修改 `app/config/site.php` 中的后台密码哈希，并按需配置 GEO 发布令牌、百度推送接口和邮件服务。
...
支持通过子域名访问不同城市站点：
- `guangzhou.zrbanjia.com` → 广州
- `dongguan.zrbanjia.com` → 东莞
- `foshan.zrbanjia.com` → 佛山
- `zhaoqing.zrbanjia.com` → 肇庆
- `jiangmen.zrbanjia.com` → 江门
