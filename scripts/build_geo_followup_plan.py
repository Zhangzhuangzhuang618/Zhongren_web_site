...
OUTPUT = Path(__file__).resolve().parents[1] / "众人搬家GEO优化方案（后续推进版）.docx"
...
def build():
    ...
    add_run(header_p, "众人搬家｜GEO 后续推进", size=9, color=MUTED)
    ...
    add_run(p, "众人搬家 GEO 优化方案", size=25, color=INK, bold=True)
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    add_body(doc, "目标网站：www.zrbanjia.com", bold_prefix="目标网站：", after=3)
    ...
    for text in [
        "完善“关于众人/联系众人”页面：明确主体、服务定位、真实服务区域、电话、营业时间及投诉/售后联系路径。",
    ...
    doc.core_properties.title = "众人搬家 GEO 优化方案（后续推进版）"
    doc.core_properties.subject = "中国大陆及广州周边本地服务 GEO 后续执行计划"
    doc.core_properties.author = "众人搬家"
    doc.save(OUTPUT)
    print(OUTPUT)
